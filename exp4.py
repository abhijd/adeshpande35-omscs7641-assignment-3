...
def write_to_csv(
    dataset_name, t_time,q_time,cv_score, acc_score
):
    fname = "files/part4/{}_metrics.csv".format(dataset_name)
    try:
        f = open(fname)
    except IOError:
        f = open(fname, "a+")
        f.write(
            "Train Time, Test Time,CV Score,Test Acc Score\n"
        )
    finally:
        f.close()
...
def runAll(X, y, dataname, ncPCA, ncICA, ncRP, ncSVD):
    ...
    accscore = accuracy_score(y_te, y_pr)
    print("Test Accuracy: " + str(accscore))
    write_to_csv(dataname+"_PCA_MLP", t_time,q_time,cvscore,accscore)
    print("done w PCA_MLP")
    ...
    accscore = accuracy_score(y_te, y_pr)
    print("Test Accuracy: " + str(accscore))
    write_to_csv(dataname+"_ICA_MLP", t_time,q_time,cvscore,accscore)
    print("done w ICA_MLP")
    ...
    accscore = accuracy_score(y_te, y_pr)
    print("Test Accuracy: " + str(accscore))
    write_to_csv(dataname+"_RP_MLP", t_time,q_time,cvscore,accscore)
    print("done w RP_MLP")
    ...
    accscore = accuracy_score(y_te, y_pr)
    print("Test Accuracy: " + str(accscore))
    write_to_csv(dataname+"_SVD_MLP", t_time,q_time,cvscore,accscore)
    print("done w SVD_MLP")
...
def run():
    ...
    ##KMeans PCA
    #liver patients
    runAll(liv_full, liv_ans_full.ravel(), "liver_patients", 2, 7, 10,  1)
    #bm
    runAll(bm_full, bm_ans_full.ravel(), "bm", 2, 12, 2,  1)
